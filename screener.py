...
class CryptoScreener:
    def __init__(self, data_map, btc_df):
        self.data = data_map
        self.btc = btc_df # 這裡已經是 DatetimeIndex
        self.results = []
    ...
    def run(self):
        ...
        # --- Step 4: RS Calculation ---
        rs_scores = []
        
        for symbol in valid_symbols:
            df = processed_data[symbol]
            
            # [修正] 直接對齊，因為現在 df 和 self.btc 都是 DatetimeIndex
        ...
        for idx, row in rs_df.iterrows():
            ...
            for L in [24, 48, 72, 96, 168]:
                ...
                if (width_pct < config.VCP_WIDTH_HISTORY_RANK and 
                    ...
                    if df['close'].iloc[-L:].min() > ema200_1h.iloc[-1]: 
                         setup_vcp = True
            ...
            if rs_rank > 85 and row['rs_trend_ok'] and (setup_vcp or setup_pp):
                ...
                bucket = "PowerPlay"
            elif rs_rank > 60 and row['rs_trend_ok']:
                bucket = "Turning"
